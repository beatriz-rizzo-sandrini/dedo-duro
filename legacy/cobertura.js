...
window.addEventListener("load", () => {
  ...
  Promise.all([
    fetch(urlEstoque).then(r => r.text()),
    fetch(urlVendas).then(r => r.text())
  ])
...
function exportarTabelaCobertura() {
  ...
  a.setAttribute("download", "cobertura.csv");
...
document.addEventListener("DOMContentLoaded", () => {
  const btnDetalhado = document.getElementById('btn-exportar-detalhado');
  if (btnDetalhado) {
    btnDetalhado.addEventListener('click', () => {
      console.log("Exportando detalhado (teste sem filtro)...");
      exportarCoberturaDetalhada();
    });
  }
});

function exportarCoberturaDetalhada() {
  const linhasDetalhadas = [["SKU", "Descrição", "Local", "Vendas", "Estoque"]];
  ...
  vendasRows.forEach(r => {
    ...
    const local = (r.c[1]?.v || "").toUpperCase().trim();
    const qtd = r.c[4]?.v || 0;
    linhasDetalhadas.push([sku, desc, local, qtd, ""]);
  });
  ...
  console.log("Gerando detalhado:", linhasDetalhadas.length);
  const csvContent = "\uFEFF" + linhasDetalhadas.map(e => e.map(cell => String(cell).replace(/;/g, ",")).join(";")).join("\n");
  const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.setAttribute("href", url);
  a.setAttribute("download", "cobertura_detalhado.csv");
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
}
