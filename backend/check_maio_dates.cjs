...
async function run() {
  console.log('Analyzing unique dates in MAIO tab...');
  const url = `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/gviz/tq?tqx=out:json&sheet=MAIO`;
  ...
  try {
    ...
    // Sort dates by count descending or print them
    console.log('Unique dates in MAIO and their row counts:');
    Object.entries(dateCounts).sort((a,b) => b[1] - a[1]).slice(0, 10).forEach(([date, count]) => {
