...
async function run() {
  ...
  const queryPaths = [
    'erp_faturamento_senior_g5/pedido/entities/e120ped',
    'erp_faturamento_senior_g5/entities/e120ped',
    'erpx_faturamento/entities/e120ped',
    'erpx_faturamento/pedido/entities/e120ped',
    'erpx_ven/entities/e120ped',
    'erp_ven/entities/e120ped'
  ];

  for (const qPath of queryPaths) {
    console.log(`2. Consultando PEDIDOS na Senior usando rota "${qPath}"...`);
    try {
      // Trazendo 5 pedidos para ver se funciona
      const queryUrl = `https://api.senior.com.br/${qPath}?$top=5`;
      ...
      console.log('================ DADOS DO PEDIDO (SENIOR) ================');
      if (productRes.data && (productRes.data.contents || productRes.data.length > 0)) {
        console.log(JSON.stringify(productRes.data.contents || productRes.data, null, 2));
      } else {
