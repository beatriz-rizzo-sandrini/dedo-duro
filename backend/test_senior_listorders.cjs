...
async function run() {
  ...
  console.log(`2. Consultando PEDIDOS na Senior usando rota erpx_com_ven/pedido/listOrders...`);
  try {
    const queryUrl = `https://api.senior.com.br/erpx_com_ven/pedido/queries/listOrders`;
    
    // Conforme o Swagger:
    // pageRequest: { offset: 0, size: 5 }
    const payload = {
      pageRequest: {
        offset: 0,
        size: 5
    ...
    console.error(`❌ Erro na rota listOrders:`);
    if (error.response) {
